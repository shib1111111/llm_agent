  ...
  <div class="db-query-container">
    ...
    <div class="chat-container">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="chat-title">Direct Database Query</h2>
      ...
      <div class="chat-box">
        <div v-for="message in messages" :key="message.id" class="message" :class="{ 'user-message': message.query }">
          ...
          <div v-if="message.response" class="bot-response">
            <strong>DB:</strong> {{ message.response }}
            <small class="text-muted">{{ message.timestamp }}</small>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    messages() {
      return this.store.state.chatMessages.filter(m => m.type === 'db');
    },
  },
  methods: {
    async connectDB() {
    ...
    async sendQuery() {
      try {
        console.log('Sending DB query:', this.query);
        await this.store.dispatch('queryDB', this.query);
